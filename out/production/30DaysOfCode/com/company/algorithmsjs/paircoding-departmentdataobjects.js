...
Step 3: Create an empty departmentObjects and employeeObjects arrays. These will hold the object your program will create

Step 4: Create a solution to compiling all the data from the four 2D arrays and creating objects from them. The object 

Part A:
      Simply take the data that is already present in the 2D arrays and add them into an object the object structures should be as follows;
*/
...
function departmentData() {
    ...
    for (let i = 0; i < departments.length; i++) {
        ...
        for (let j = 0; j < employeeDepartment.length; j++) {
            ...
            if (departments[i][0] == employeeDepartment[j][1] && employeeDepartment[j][3] == "9999-01-01") {

                for (let k = 0; k < employees.length; k++) {

                    if (employeeDepartment[j][0] == employees[k][0]) {
                        count++
                    }
                    if (employeeDepartment[j][0] == employees[k][0] && employees[k][4] == "M") {
                        maletotal++

                    } else if (employeeDepartment[j][0] == employees[k][0] && employees[k][4] == "F") {
                        femaleTotal++
                    }
                }
            }
        }
        const companyDepartments = { id: departments[i][0], name: departments[i][1], totalEmployees: count, genders: { male: maletotal, female: femaleTotal } }
        departmentObjects.push(companyDepartments)
    }
    console.log(departmentObjects);
}


// employeeObjects attempt #1
...
//             if (employees[i][0] == employeeDepartment[j][0] && employeeDepartment[j + 1] != undefined && employeeDepartment[j][0] != employeeDepartment[j + 1][0]) {


//                 const workerInfo = {
//                     id: employees[i][0],
//                     name: `${employees[i][2]} ${employees[i][3]}`,
//                     gender: employees[i][4],
//                     birthday: employees[i][1],
//                     hireDate: employees[i][5],
//                     isStillEmployed: employeeDepartment[j][3] == "9999-01-01",
//                 }
...
for (let i = 0; i < employeeDepartment.length; i++) {
    ...
    const depot = employeeDepartment[i][1]

    if (employeeDepartment[i + 1] != undefined && employeeDepartment[i][0] != employeeDepartment[i + 1][0]) {

        for (let j = 0; j < employeeSalaries.length; j++) {

            if (employeeSalaries[j][0] == employeeDepartment[i][0] && employeeSalaries[j][3] == "9999-01-01") {

                const empIdx = employeeSalaries[j][0] - 10001
                const dpId = depot[3] - 1

                const empInfo = {
                    id: employeeDepartment[i][0],
                    fullName: `${employees[empIdx][2]} ${employees[empIdx][3]}`,
                    gender: employees[empIdx][4],
                    birthday: employees[empIdx][1], hireDate: employees[empIdx][5],
                    isStillEmployed: employeeSalaries[j][3] == "9999-01-01",
                    departmentsWorkedFor: departments[dpId][1],
                    mostRecentSalary: employeeSalaries[j][1].toLocaleString()
                }
                employeeObjects.push(empInfo)
            }
        }
    }
}
...
function employeeData() {
    ...
    for (let i = 0; i < employees.length; i++) {
        ...
        for (let j = 0; j < employeeDepartment.length; j++) {
            ...
            if (employees[i][0] == employeeDepartment[j][0] && employeeDepartment[j][3] == "9999-01-01") {
                employed = true

            } else if (employees[i][0] == employeeDepartment[j][0] && employeeDepartment[j][3] != "9999-01-01") {
                employed = false
            }
        }
        const workerInfo = {
            ...
            birthday: employees[i][1],
            hireDate: employees[i][5],
            isStillEmployed: employed,
        }
        empData.push(workerInfo)
    }
    console.log(empData);
}